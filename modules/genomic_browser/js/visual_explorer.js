...
var myModule = (function () {
    ...
    function loadChromosomes() {
        ...
        switch(genomicData.organism) {
            case "Human":
                var chrom =['chr1','chr2','chr3','chr4','chr5','chr6','chr7','chr8','chr9','chr10','chr11','chr12','chr13','chr14','chr15','chr16','chr17','chr18','chr19','chr20','chr21','chr22','chrX','chrY']; 
                for( i in chrom ) {
    ...
    function updateChromosomesSelect() {
        getChromosomesLabels().forEach( function (label, index){
            var opt = document.createElement("option");
            opt.value = label;
            opt.text = label;
            chromosomeInput.add(opt)
        });
    }
...
// memory_limit = 128M --> changed to 1024M
    ...
    function loadGenes(chromosomeLabel) {
        $.ajax({
            ...
            data: {fileName: "genes_" + chromosomeLabel + ".tsv"},
            type: 'post',
            //url: "AjaxHelper.php?Module=genomic_browser&script=GetReference.php&fileName=genes_" + chromosomeLabel + ".tsv",
...
*/    
    ...
    function loadCpgs(chromosomeLabel) {
        $.ajax({
            url: "AjaxHelper.php?Module=genomic_browser&script=getReference.php&fileName=/cpg450k_" + chromosomeLabel + "_pos.tsv",
            success: function(data) {
                // TODO Add validation on the headers. transcripts are optionnal.
                // split the whole text per line and remove the first line (headers)
                var lineArray = data.split('\n').splice(1);
    ...
    return {
        init: function() {
            ...
            chromosomeInput = document.getElementById("chr");
            geneInput = document.getElementById("gene");
            coordinatesInput.startLoc = document.getElementById("start");
            coordinatesInput.endLoc = document.getElementById("end");
            updateButton = document.getElementById("update");
            fileInput = document.getElementById("input_file_beta_values");
            // TODO Add validation on the présence of the inputs.
