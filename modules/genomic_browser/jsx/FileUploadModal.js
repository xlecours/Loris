...
var GenomicFileUploadModal = React.createClass({
  ...
  render: function() {
    ...
    if (this.state.submited) {
      footerButtons.push(<button className="btn btn-default" onClick={this.reloadPage} data-dismiss="modal" key="b1">Ok</button>);
    } else {
      if (this.state.readyForUpload) {
        footerButtons.push(<button className="btn btn-primary" onClick={this.handleUploadSubmit} role="button" aria-disabled="false" key="b1">Upload</button>);
      }

      footerButtons.push(<button className="btn btn-default" id="cancelButton" role="reset" type="reset" data-dismiss="modal" key="b2">Cancel</button>);
    }
    return (
...
var UploadForm = React.createClass({
  ...
  render: function() {
    ...
    inputs.push(<FileTypeSelect baseURL={this.props.baseURL} multiple={false} onFileTypeChange={this.handleFileTypeChange} name="fileType" label="File type:" key="fileType"/>);

    switch (this.state.fileType) {
      case 'Methylation beta-values':
        inputs.push(<FileInput name="fileData" label="File :" key="FileInput1"/>);
        inputs.push(<TextAreaInput name="description" label="Description :" key="TextAreaInput"/>);
        if (!this.state.useColumnHeaders) {
          inputs.push(<FileInput name="fileMapping" label="Mapping :" key="FileInput2"/>);
        }
        inputs.push(<CheckboxInput handleChange={this.handleCheckboxChange} checked={this.state.useColumnHeaders} name="pscidColumn" key="CheckboxInput" label="."/>);
        inputs.push(<ProgressBar name="progressbar" label="Progress :" key="ProgressBar"/>);
        break;
      case 'Other':
        inputs.push(<FileInput name="fileData" label="File :" key="FileInput1"/>);
        inputs.push(<TextAreaInput name="description" label="Description :" key="TextAreaInput"/>);
...
var FileTypeSelect = React.createClass({
  ...
  render: function() {
    var options = this.state.availableFileType.map(function(e,k) {
      return (<option value={e.genomicFileType} key={k}>{e.genomicFileType}</option>);
    }
        );
  ...
  render: function() {
    ...
    return (
            <div className="form-group col-sm-12">
                ...
                <div className="col-xs-9">
                    // <input className="user-success" name={this.props.name} id={this.props.name} type="checkbox" readOnly="true" checked="true" style={{'marginRight': '1em'}} />
                </div>
            </div>
        );
  }
