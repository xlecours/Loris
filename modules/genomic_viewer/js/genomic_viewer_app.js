...
class Gene extends React.Component {
   ...
   */
  draw(genomicRange) {
    const canvas = this.refs.thatCanvas.getDOMNode();
    if (canvas) {
...
class GeneTrack extends React.Component {
  ...
  addBackground() {
    const forwardCanvas = this.refs.forwardBackgroundCanvas.getDOMNode();
    const reverseCanvas = this.refs.reverseBackgroundCanvas.getDOMNode();
  ...
  render() {
    ...
    let genes = this.state.genes.map(function (g) {
      ...
      const exonStarts = g.exonStarts.split(',').map(function (e) {
        return parseInt(e);
      });
      const exonEnds = g.exonEnds.split(',').map(function (e) {
        return parseInt(e);
      });
...
class BetaValueDistribution extends React.Component {
  ...
  drawBox() {
    ...
    let boxPlot = d3.box().whiskers(this.iqr(1.5)).width(boxWidth).height(100);

    boxPlot.domain([0, 1]);

    // Binding data the the svg element group
    const g = d3.select('#' + this.props.cpgName).data([this.props.betaValues]);
...
class CPGTrack extends React.Component {
  ...
  render() {
    ...
    const yAxis = React.createElement(
      "svg",
    ...
    const lines = [React.createElement("line", { ref: "hypo", x1: "0", y1: "80", x2: "0", y2: "80" }), React.createElement("line", { ref: "hyper", x1: "0", y1: "40", x2: "0", y2: "40" })];
...
class SNPTrack extends React.Component {
  ...
  onClick(event) {
    ...
    let element_name = event.target.textContent;
    this.refs[element_name].getDOMNode().classList.add('focus');
...
class ChIPPeakTrack extends React.Component {
  render() {
