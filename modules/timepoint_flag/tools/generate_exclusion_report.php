...
get the list of exclusions (array ExclusionData: Exclusion_name(Name)
  =>Flag_type(Type))
...
*/
$query = "Select Submitted, MRIQCStatus, MRIQCPending, Scan_done,  Approval, "
        . "BVLQCStatus, BVLQCType, ID as SessionID FROM session WHERE Active = 'Y' "
        . "AND Screening = 'Pass' AND VisitNo=1";
...
print "Site\tDCCID\tPSCID\tSessionID\tVisit\tObjective\tAge at Visit\tACCEPTED\t"
. "Date_of_visit\tIQTest_date\tDB_Window\tSent To DCC\tMR Scan Done\tQC Completed\t"
        . "MRI QC\tMRI QC Pending\tBVL QC\tBVL QC Type\tCertification\t"
        . "Adverse Findings\tFeedback";
...
foreach ($result as $currentTimepoint) {
    ...
    */
    $query = "SELECT c.DoB, c.EDC, c.PSCID, s.Date_visit, w.Date_taken as WASI_date,"
            . "d.Date_taken as DAS_date FROM candidate as c LEFT JOIN session as s "
            . "USING (CandID) LEFT JOIN flag as f ON (s.ID = f.SessionID) "
            . "LEFT JOIN wasi as w ON (f.CommentID = w.CommentID) "
            . "LEFT JOIN das as d ON (f.CommentID = d.CommentID) "
            . "WHERE f.Test_name IN ('wasi', 'das') AND s.VisitNo=1 "
            . "AND c.CandID=".$data['DCCID'];
    ...
    if (empty($data['Date_IQ_Test']) || empty($data['Date_visit'])) {
        ...
        if (Utility::_checkDate($dateOfIQTest) == false
            ...
            if (strtolower($data['Visit']) == 'v1') {
                ...
                $data['DB_Window'] = abs(
                    ($dateOfVisit['Y']*12 + $dateOfVisit['M'])
                    - ($dateOfIQTest['Y']*12 + $dateOfIQTest['M'])
                );
            }
        }
        unset($dateOfIQTest);
    ...
    */
    ...
    foreach ($listOfFlags as $flagName) {
        ...
        if ($indexCount == 1) {
            ...
            $listOfTriggers[$flagName] = $timepointFlagObject
                    ->getListOfTriggers($flagName);


            if (is_array($listOfTriggers[$flagName])) {

                // print the flag and trigger labels (once)
                // - for flags that have triggers
        ...
        if (is_array($listOfTriggers[$flagName])) {
            ...
            $listTriggerFieldValues = $timepointFlagObject
                    ->getTriggerFieldValues($flagName);

            // add to data array
            $data[$flagName] = $flagData;

            foreach ($listOfTriggers[$flagName] as $trgName) {
                $data[$flagName][$trgName] = $listTriggerFieldValues[$trgName];
            }

            // get the array of review fields w/ field values
            $reviewfieldArray = $timepointFlagObject
                    ->getFlagReviewFieldsValues($flagName);
            ...
            if (is_array($reviewfieldArray) && count($reviewfieldArray) > 0) {
                foreach ($reviewfieldArray as $reviewFieldLabel=>$reviewFieldValue) {
                    ...
                    if (!empty($reviewFieldValue)) {
                        $data[$flagName]['Review'] .= "($reviewFieldLabel): ".
                                trim($reviewFieldValue) ."; ";
                    }
                }
            }
    ...
    if ($currentTimepoint['Scan_done'] == 'Y') {
        $adverseArray = array();
        $query        = "SELECT Findings_confirmed FROM NIH_PD_DEV.mri_adverse "
                . "WHERE SessionID = '$sessionID' "
                . "AND Findings_confirmed IS NOT NULL GROUP BY SessionID";
        $db->selectRow($query, $adverseArray);
        // if the ID was returned then there was a findings
        if (count($adverseArray) == 0
            || empty($adverseArray['Findings_confirmed'])
        ) {
            // adverse finding DNE
    ...
    */
    ...
    if ($data['Submitted'] == 'Y'
        && !in_array($data['Feedback'], array('opened', 'answered'))
        && $data['BVLQCStatus'] == 'Complete'
        &&  ($data['Scan_done'] == 'N' || $data['Scan_done'] == 'Y'
        && !empty($data['MRIQCStatus']) && $data['MRIQCPending'] == 'N')
    ) {
        $data['QCCompleted'] = 'Y';
    } else {
        $data['QCCompleted'] = 'N';
...
function dumpLine($instrumentList, $listOfFlags, $listOfTriggers, $sessionID, $data)
{
    ...
    // dump the line
    print $data['Site'].$TAB.$data['DCCID'].$TAB.$data['PSCID'].$TAB.
            $data['SessionID'].$TAB.$data['Visit'].$TAB.$data['Objective'].$TAB.
            $data['Age_Visit'].$TAB.$data['Approval'].$TAB.$data['Date_visit'].$TAB.
            $data['Date_IQ_Test'].$TAB.$data['DB_Window'].$TAB.$data['Submitted'].
            $TAB.$data['Scan_done'].$TAB.$data['QCCompleted'].$TAB.
            $data['MRIQCStatus'].$TAB.$data['MRIQCPending'].$TAB.
            $data['BVLQCStatus'].$TAB.$data['BVLQCType'].$TAB.
            $data['Certification'].$TAB.$data['Adverse_findings'].$TAB.
            $data['Feedback'];
