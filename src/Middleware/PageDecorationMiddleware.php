 ...
 * @link     https://www.github.com/aces/Loris/
 */
class PageDecorationMiddleware implements MiddlewareInterface, MiddlewareChainer
{
    ...
    public function __construct(\User $user, \NDB_Config $config)
    {
        $this->user   = $user;
        $this->config = $config;
    }
    ...
    public function process(
        ...
        if ($request->getHeader('Accept') != 'application/json') {
            if ($this->user instanceOf \LORIS\AnonymousUser) {
                 $decorator = new \LORIS\Middleware\AnonymousPageDecorationMiddleware('', $this->config, [], []);
            } else {
                 $decorator = new \LORIS\Middleware\UserPageDecorationMiddleware($this->user, '', '', $this->config, [], []);
            }
            
        }
        return $decorator->withMiddleware($this->next)->process($request, $handler);
    }
}
