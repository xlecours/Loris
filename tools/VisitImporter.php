...
class VisitImporter extends DataImporter {
    ...
    public function __construct(
        ...
        $result = \Database::singleton()->pselect(
            'SELECT DISTINCT CandID FROM session',
            array()
        );
        foreach ($result as $row) {
            $rez = \Database::singleton()->pselect(
                'SELECT Visit_label FROM session WHERE CandID=:cid',
                array('cid'=>$row['CandID'])
            );
            $list = [];
            foreach ($rez as $r) {
                $list[] = $r['Visit_label'];
            }
            $this->existingSessions[$row['CandID']] = $list;
        }
    ...
    function buildSQLQuery(array $row) {
        ...
        if (empty($this->existingSessions[$newCandID])
            ...
            unset($command['data']['Visit_label']);
