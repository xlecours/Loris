...
$filepath = "/var/www/neurodb/" . OUTPUT_FOLDER;
...
if ($mode === VISIT_EXPORT)
{
    ...
    $filepath = isset($filepath) ? $argv[3] : $filepath;
...
//remove the commented out columns and the `Age_At_MRI is not null` condition when using this "visits" flag for other purposes
/*        'CenterID',
        'VisitNo',
*/        'Visit_label',
/*        'SubprojectID',
...
'Age_At_MRI'
    ...
    $query = "SELECT " .
        ...
        "AND DATE(Date_visit) < :cutoffDate and Age_At_MRI is not null";
    ...
    //
    // Overwrite default path if a custom path was specified by user.
    $filepath = isset($filepath) ? $argv[5] : $filepath;

    // Format of output filename: <table_column_dataExtract_output.csv>
    $filename = 
        "{$table}_dataExtract_output.csv";
...
//including "empty" instruments so numbers match more appropriately
            WHERE (DATE(t.Date_taken) < :cutoffDate or date_taken is null)" .
            'AND f.CommentID NOT LIKE "DDE%" and (visit_label like "PRE%" or visit_label like "NAP%") group by c.PSCID ';
 ...
 * @return void
 */
function writeToCsv(SplFileInfo $file, array $headers, array $data) {
 ...
 * @return array
 */
function prependTableAbbreviation(array $columnNames, $prefix) {
    $result = array();
